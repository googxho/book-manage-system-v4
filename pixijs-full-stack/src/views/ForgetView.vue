...
const step = ref(0) // 0: 验证邮箱, 1: 重置密码
const email = ref('')
const code = ref('')
const password = ref('')
...
function sendCode() {
  ...
  errorMsg.value = ''
  get(`/api/auth/ask-code?email=${encodeURIComponent(email.value)}&type=reset`, () => {
    ...
    <div class="form-overlay">
      <div class="form-card">
        <div class="form-header">
          ...
          <p class="form-subtitle">{{ step === 0 ? '验证您的邮箱' : '设置新密码' }}</p>
        </div>
        ...
        <div class="step-indicator">
          ...
          <div class="step-dot" :class="{ active: step === 1 }">2</div>
        </div>

        <!-- 步骤 0: 验证邮箱 -->
        <form v-if="step === 0" @submit.prevent="handleConfirm" class="form-body">
          <div class="input-group">
            <label for="email">邮箱</label>
            <div class="input-with-btn">
              <input id="email" v-model="email" type="email" placeholder="请输入注册时使用的邮箱" />
        ...
        <!-- 步骤 1: 重置密码 -->
        <form v-else @submit.prevent="handleReset" class="form-body">
          <div class="input-group">
            <label for="password">新密码</label>
            <input id="password" v-model="password" type="password" placeholder="6-20位新密码" maxlength="20" autocomplete="new-password" />
          </div>

          <div class="input-group">
            <label for="passwordRepeat">确认密码</label>
            <input id="passwordRepeat" v-model="passwordRepeat" type="password" placeholder="再次输入新密码" maxlength="20" autocomplete="new-password" />
          </div>
        ...
        <div class="form-links">
          <router-link to="/" class="form-link">返回登录</router-link>
        </div>
